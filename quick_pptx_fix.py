#!/usr/bin/env python3
"""
Quick fix for the pptx_processor import issue
Save this as quick_pptx_fix.py and run it
"""

from pathlib import Path

def main():
    """Replace the problematic pptx_processor with a working version"""
    
    pptx_file = Path('src/core/pptx_processor.py')
    
    # Create backup
    backup_file = pptx_file.with_suffix('.py.backup_final')
    if pptx_file.exists():
    ...
    # Working processor content using only built-in exceptions
    working_content = '''"""
PowerPoint processor for PPTrans - working version with built-in exceptions only
"""
...
'''
    ...
    # Write the working content
    with open(pptx_file, 'w', encoding='utf-8') as f:
        f.write(working_content)
    
    print(f"Replaced {pptx_file} with working version")
    ...
    try:
        ...
        print("SUCCESS: PPTXProcessor imports and works!")
